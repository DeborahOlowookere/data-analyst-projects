...
CREATE TABLE `Suppliers` (
    ...
    CONSTRAINT `uc_Suppliers_Email` UNIQUE (`Email`)
) ENGINE=InnoDB;
...
-- to identify all inactive suppliers
SELECT sp.supplier_name, sp.status
FROM Suppliers sp
...
CREATE TABLE `Customers` (
    ...
    `Customer_email` VARCHAR(100) NOT NULL,  -- Correct column name
    ...
    CONSTRAINT `uc_Customers_Email` UNIQUE (`Customer_email`)
);
...
SELECT oi.drugid, oi.total_price
FROM Order_items oi
where oi.orderid = 'o8';
...
-- Create a view that uses at least 3-4 base tables; prepare and demonstrate a query that uses the view to produce, 
-- a logically arranged result set for analysis

USE Pharmacy;
CREATE OR REPLACE VIEW vw_analysis AS
SELECT DISTINCT 
    customer_name, customer_status, order_item_id, quantity_ordered, unit_price, 
    OI.total_price, s.supplierID, Supplier_Name, rating, lead_time, Status, 
    d.drugid, drug_name, class, purchaseid, quantity_purchased, price_per_unit, 
    total_cost, quantity_in_stock, restock_threshold, last_restocked, purchase_date
FROM suppliers AS S
...
Left JOIN inventory AS INV
   ON INV.DrugID = d.DrugID
Left JOIN order_items AS OI
	ON OI.DrugID = d.DrugID
...
-- find the average total spending by a customer and average quantity of drugs bought by a customer
SELECT customer_name, sum(total_price), sum(quantity_ordered)
FROM customer_order_analysis
GROUP BY customer_name;

-- find the average quantity of drugs supplied that has been ordered
SELECT supplier_name, supplierid, sum(quantity_ordered)
FROM vw_analysis
GROUP BY supplier_name, supplierid
ORDER BY supplierid asc;

-- find the most drug bought by a customer

SELECT DISTINCT drug_name, sum(quantity_ordered)
FROM customer_order_analysis
GROUP BY drug_name
...
GROUP BY Customer_name, Drug_name, Supplier_name
...
ORDER BY Total_Spending DESC;
 ...
 -- subquery below used to find out suppliers by id and name who supply drugs with total_amount of 500 and unit_price is 
 -- greater than or equal to 200 and less than or equal to 50
...
-- This function automates average sale calculations for any date range
...
SELECT average_sale_by_date ('2025-01-10', '2025-01-16') AS avg_sale;

-- In your database, create a stored procedure and demonstrate how it runs
-- call this stored procedure to insert or update an inventory record, depending on whether the record already exists.

use pharmacy;
DELIMITER //
CREATE PROCEDURE InsertInventory(
    new_InventoryID VARCHAR(100),
    new_DrugID VARCHAR(100),
    new_Quantity_in_stock INT,
    new_last_restocked DATE,
    restock_threshold INT,
    new_batch_no VARCHAR(100),
...
BEGIN
    ...
    ELSE
        UPDATE inventory
        SET Quantity_in_stock = Quantity_in_stock + new_Quantity_in_stock
        WHERE DrugID = new_DrugID AND InventoryID = new_InventoryID;
    END IF;
END //
...
-- Calling out procedure demonstration
select * from inventory;
CALL InsertInventory('INV002', 'D2', 20, '2025-02-01', 5, 'BATCH003', '2026-01-15');
select * from inventory;     
CALL InsertInventory('I1', 'D1', 25, '2025-02-02', 10, 'BATCH004', '2025-12-31');
...
CREATE EVENT show_expiring_drugs2
...
DO
  ...
  SELECT * FROM expiring_drugs_log;
SHOW EVENTS;
SELECT * FROM information_schema.EVENTS WHERE EVENT_NAME = 'show_expiring_drugs';
...
-- Demonstrating code
INSERT INTO Inventory (InventoryID, DrugID, Quantity_in_stock, Last_restocked, Restock_threshold, Batch_no, Expiry_date)
VALUES ('I100', 'D100', 200, '2025-02-01', 50, 'BATCH500', '2026-12-31');

SELECT * FROM Drugs WHERE DrugID = 'D100';